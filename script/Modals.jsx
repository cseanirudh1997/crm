...
export function ServiceRequestModal({ session, onClose }) {
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    if (!form.serviceType || !form.description.trim()) {
    ...
    >
      {success ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Service type */}
          <div>
            <label className="block text-xs text-gray-400 mb-1.5 font-medium">Service Type *</label>
            <select
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-gray-400 mb-1.5 font-medium">Contact Email</label>
              <input type="text" value={session?.email || ''} readOnly className="input-field text-sm opacity-60 cursor-not-allowed" />
            </div>
            <div>
          ...
          {/* Urgency */}
          <div>
            <label className="block text-xs text-gray-400 mb-2 font-medium">Urgency</label>
...
export function SupportRequestModal({ session, onClose }) {
  ...
  const PRIORITY_OPTIONS = [
    { value: 'low',      label: 'Low',      color: 'text-gray-400',   border: 'border-gray-700'   },
    { value: 'medium',   label: 'Medium',   color: 'text-amber-300',  border: 'border-amber-700'  },
    { value: 'high',     label: 'High',     color: 'text-orange-300', border: 'border-orange-700' },
    { value: 'critical', label: 'Critical', color: 'text-red-300',    border: 'border-red-700'    },
  ]
    ...
    >
      {success ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Description */}
          <div>
            <label className="block text-xs text-gray-400 mb-1.5 font-medium">Details *</label>
            <textarea
              value={form.description}
              onChange={(e) => update('description', e.target.value)}
              placeholder="Steps to reproduce, error messages, affected service, impact…"
              rows={4}
              className="input-field text-sm resize-none"
              required
            />
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {PRIORITY_OPTIONS.map(({ value, label, color, border }) => (
                <button
                  key={value}
                  type="button"
