...
export default function AdminDashboard({ session }) {
  ...
  async function loadData() {
    setLoading(true)
    const m = await getEnterpriseMetrics(session?.email)
  ...
  const ADMIN_METRICS = [
    { label: 'Total Requests',     value: metrics?.totalRequests    || '–', change: '+12',  up: true,  icon: Activity,  color: 'from-brand-600 to-brand-800',   bg: 'bg-brand-900/20',  border: 'border-brand-700/30' },
    { label: 'Resolved',           value: metrics?.resolvedRequests || '–', change: '+8',   up: true,  icon: CheckCircle, color: 'from-emerald-600 to-emerald-800', bg: 'bg-emerald-900/20', border: 'border-emerald-700/30' },
    ...
    { label: 'Cost Saved (Total)', value: metrics?.costSaved        || '–', change: '+$400K', up: true, icon: TrendingUp, color: 'from-teal-600 to-teal-800',   bg: 'bg-teal-900/20',   border: 'border-teal-700/30'  },
  ]
