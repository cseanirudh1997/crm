  ...
  return (
    <div className="flex items-center gap-1 px-4 py-3 rounded-2xl rounded-tl-sm bg-white/10 border border-white/10 w-16">
      {[0, 0.15, 0.3].map((delay) => (
        <motion.div key={delay} className="w-2 h-2 rounded-full bg-gray-400" animate={{ y: [0, -5, 0] }} transition={{ duration: 0.6, repeat: Infinity, delay }} />
      ))}
    </div>
  )
}
...
export default function Chatbot() {
  ...
  async function sendMessage(text) {
    ...
    setInput('')
    setMessages((m) => [...m, buildUserMessage(query)])
    setLoading(true)
    try {
  ...
  return (
    <>
      <motion.div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 2, type: 'spring', stiffness: 260, damping: 20 }}>
        {showDot && !open && (
          ...
          >
            <div className="glass-dark border border-white/10 rounded-2xl shadow-glass flex flex-col overflow-hidden" style={{ maxHeight: 'min(540px, calc(100vh - 8rem))' }}>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
                {messages.map((msg) => (
                  <motion.div key={msg.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex flex-col gap-1 ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
              ...
              {!loading && (
                <div className="px-4 pb-2 flex flex-wrap gap-2">
                  {SUGGESTED_PROMPTS.slice(0, 3).map((p) => (
                    <button key={p} onClick={() => sendMessage(p)} className="text-xs px-2.5 py-1.5 rounded-full bg-white/5 border border-white/10 text-gray-300 hover:bg-white/10 hover:text-white transition-all">
                      {p}
                    </button>
                  ))}
                </div>
